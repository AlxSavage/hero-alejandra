...
export default function(event, template) {
  let name = template.$(event.target).closest('.side-panel-profile').data('panel-template');
  let user = template.$(event.target).closest('.side-panel-profile').data('panel-user');
